    ...
    $('.savedArticles').on("click", () => { // Query for Saved Articles
      ...
      fetch("/api/savedArticles", {method: "GET"}).then(response => response.json()).then((response) => {
        ...
        response.map(article => {
          let articleDiv = "<li id='" + article["_id"] + "' data-url='" + article.url  + "'class='circle'><span class='title'>" + article.title + "</span><p>" + article.tagline + "</P><a class='secondary-content deleteArticle'><i class='material-icons hoverRed'>delete_forever</i></a></li>";
